...
import {
  mockElevators, mockContracts, mockJobs, formatDate
} from "@/lib/mock-data";
...
// Demo: customer "u-cus-1" maps to customer "c-1" (Vinhomes)
const CUSTOMER_ID = "c-1";

function PortalIndex() {
  useAppStore(); // ensure subscription
  ...
  return (
    <AppShell>
      <PageHeader title="Tổng quan của bạn" description="Thông tin thang máy, hợp đồng và lịch bảo trì" />

      <div className="grid gap-4 sm:grid-cols-3 mb-6">
        <StatCard label="Thang máy của bạn" value={myElevators.length} icon={Building2} accent="primary" />
        <StatCard label="Hợp đồng đang hoạt động" value={myContracts.filter(c => c.status === "active" || c.status === "expiring").length} icon={FileText} accent="success" />
        <StatCard label="Đến hạn bảo trì" value={dueElevators} icon={AlertTriangle} accent="warning" />
      </div>
      ...
      {upcomingJob && (
        <Card className="p-5 mb-6 border-l-4 border-l-primary bg-primary/5">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              <div className="text-xs font-medium text-primary uppercase tracking-wider">Lịch tiếp theo</div>
              <div className="mt-1 font-semibold">{upcomingJob.title}</div>
              <div className="text-sm text-muted-foreground">{formatDate(upcomingJob.scheduledFor)}</div>
            </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/portal/elevators"><Button variant="ghost" size="sm">Tất cả <ArrowRight className="h-3.5 w-3.5 ml-1" /></Button></Link>
          </div>
        ...
        <Card className="p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/portal/issues"><Button variant="ghost" size="sm">Báo lỗi <ArrowRight className="h-3.5 w-3.5 ml-1" /></Button></Link>
          </div>
