...
import { mockElevators, getProject } from "@/lib/mock-data";
    ...
    <MobileShell title="Danh sách thiết bị">
      <div className="sticky top-0 bg-slate-50/80 backdrop-blur-sm z-20 px-4 py-3 border-b">
         <div className="flex gap-2">
            ...
            <Button size="icon" className="h-[42px] w-[42px] rounded-xl bg-primary text-white shadow-lg shadow-primary/20 shrink-0">
      ...
      <div className="p-4 space-y-4">
        {filtered.map((elevator) => {
          ...
          return (
            <Card key={elevator.id} className="p-4 shadow-sm border-none bg-background active:scale-[0.98] transition-transform">
               <div className="flex gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                     <div className="flex justify-between items-start gap-2">
                        <h3 className="font-bold text-sm leading-tight line-clamp-1">{elevator.building}</h3>
                     ...
                     <div className="mt-2 space-y-1.5">
                        <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                            <span className="font-mono bg-muted px-1 rounded text-[10px]">{elevator.id}</span>
                     ...
                     <div className="mt-4 flex items-center justify-between pt-3 border-t">
                        <div className="flex items-center gap-3">
                           <div className="flex flex-col">
                              <span className="text-[8px] uppercase text-muted-foreground font-bold leading-none">Lần cuối</span>
                              <span className="text-[10px] font-bold mt-1">15/04</span>
                           </div>
                           <div className="h-4 w-px bg-slate-200" />
                           <div className="flex flex-col">
                              <span className="text-[8px] uppercase text-muted-foreground font-bold leading-none">Công việc</span>
                              <span className="text-[10px] font-bold mt-1">Bảo trì số 4</span>
                           </div>
                        </div>
                        <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full bg-slate-50 text-muted-foreground transition-colors hover:text-primary">
