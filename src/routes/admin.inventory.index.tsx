...
function InventoryPage() {
  ...
  return (
    <AppShell>
        ...
        actions={
          canModify ? (
            <div className="flex gap-2">
              ...
              >
                <ArrowRightLeft className="h-4 w-4 md:mr-1.5" />{" "}
                <span className="hidden md:inline">Chuyển kho</span>
              </Button>
              <Button
              ...
              >
                ...
                <span className="hidden md:inline">Nhập vật tư mới</span>
              </Button>
            </div>
          ) : (
            <Badge
              variant="outline"
              className="text-muted-foreground italic border-dashed px-4 py-2 bg-muted/20"
            >
              Chế độ xem tồn kho
            </Badge>
          )
        ...
        {filtered.length === 0 ? (
          <div className="py-16 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Hãy thử thay đổi từ khóa hoặc danh mục tìm kiếm
            </p>
          </div>
        ) : viewMode === "grid" ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4">
            {paged.map((i) => (
              <InventoryCard key={i.id} item={i} onReceive={() => setReceiveItem(i)} />
            ))}
          </div>
        ) : (
          <div className="divide-y border-t bg-background">
            {paged.map((i) => (
              <InventoryListItem key={i.id} item={i} onReceive={() => setReceiveItem(i)} />
            ))}
          </div>
        )}
      ...
      <TransferInventoryModal open={transferOpen} onClose={() => setTransferOpen(false)} />
    </AppShell>
  );
}
  ...
  return (
    <Card className="relative overflow-hidden border p-5 transition-transform shadow-sm">
      <div className="flex items-center justify-between z-10 relative">
        ...
        <div
          className={`flex h-12 w-12 items-center justify-center rounded-xl border ${colorStyles}`}
        >
      ...
      <div
        className={`absolute -right-6 -top-6 w-24 h-24 rounded-full blur-2xl opacity-20 ${colorStyles.split(" ")[0]}`}
      />
    </Card>
  );
}
  ...
  return (
    <div className="group flex flex-col bg-card rounded-xl border shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden">
      <div className="p-4 flex-1">
        <div className="flex justify-between items-start mb-3">
          <div className="flex items-center gap-2">
            ...
            {isLow && (
              <span className="px-2.5 py-0.5 rounded-full bg-rose-500/10 text-rose-600 dark:text-rose-400 text-xs font-bold flex items-center gap-1">
        ...
        <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-sm bg-muted/20 p-3 rounded-lg border border-muted/50">
          <div>
            ...
            <div className="font-semibold">
              {i.stock} <span className="text-xs text-muted-foreground font-normal">{i.unit}</span>
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <div
              className={`font-bold ${isLow ? "text-destructive" : "text-emerald-600 dark:text-emerald-400"}`}
            >
              {available}
            </div>
          </div>
        ...
        <div className="mt-4">
          ...
          <Progress
            value={stockPercent}
            className={`h-1.5 ${isLow ? "[&>div]:bg-rose-500" : ""}`}
          />
        </div>
      </div>
      ...
      <div className="p-3 border-t bg-muted/10 grid grid-cols-1">
        <Button
          variant={isLow ? "default" : "secondary"}
          size="sm"
          className="w-full text-xs font-medium shadow-none transition-transform active:scale-95"
          onClick={onReceive}
        >
  ...
  return (
    <div className="p-4 flex flex-col xl:flex-row xl:items-center gap-4 hover:bg-muted/30 transition-colors group">
      <div className="flex items-center gap-3 flex-1 min-w-0">
        <div
          className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border ${isLow ? "bg-rose-500/10 text-rose-500 border-rose-500/20" : "bg-primary/5 text-primary border-primary/10"}`}
        >
      ...
      <div className="flex items-center gap-6 text-sm overflow-x-auto pb-2 xl:pb-0 hide-scrollbar">
        ...
        <div>
          <Button
            variant="outline"
            size="sm"
            className="h-8 shadow-sm hover:border-primary/50 hover:bg-primary/5 transition-colors"
            onClick={onReceive}
          >
            Nhập kho
          </Button>
