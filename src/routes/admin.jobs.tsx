  ...
  return (
    <AppShell>
      <PageHeader
        ...
        actions={<Button><Plus className="h-4 w-4 mr-1.5" /> Tạo công việc</Button>}
      />

      <div className="grid gap-4 sm:grid-cols-4 mb-6">
        {Object.entries(jobStatusLabel).slice(0, 4).map(([k, v]) => (
          <Card key={k} className="p-4">
            <div className="text-xs text-muted-foreground">{v}</div>
            <div className="mt-1 text-2xl font-bold">{mockJobs.filter(j => j.status === k).length}</div>
          </Card>
        ))}
      </div>
      ...
      <Card className="overflow-hidden">
        <div className="flex flex-col sm:flex-row gap-3 p-4 border-b">
          <div className="flex-1 relative">
            ...
            <Input placeholder="Tìm công việc..." className="pl-9" value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} />
          </div>
          ...
          <Select value={statusFilter} onValueChange={(v) => { setStatusFilter(v); setPage(1); }}>
            ...
            <SelectContent>
              ...
              {Object.entries(jobStatusLabel).map(([k, v]) => <SelectItem key={k} value={k}>{v}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
        ...
        <div className="divide-y">
          {paged.map(j => {
            ...
            return (
              <Link key={j.id} to="/admin/jobs/$jobId" params={{ jobId: j.id }} className="block p-4 hover:bg-muted/30 transition-colors">
                  ...
                  <div className="flex items-start gap-3 flex-1 min-w-0">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className="text-[10px] px-2 py-0.5 rounded-full bg-muted text-muted-foreground">{typeLabel[j.type]}</span>
