...
export const Route = createFileRoute("/tech/jobs/$jobId/maintenance")({
  loader: ({ params }) => {
...
export function MaintenanceWizard({ job }: { job: Job }) {
  ...
  const handleStepResult = (result: 'ok' | 'repair' | 'replace') => {
    ...
    newSteps[currentStepIdx] = { 
      ...newSteps[currentStepIdx], 
      result, 
      resolved: result !== 'replace' // replace needs part selection to resolve
    };
    setSteps(newSteps);
  ...
  const handleFinish = () => {
    ...
    navigate({ to: `${prefix}/tech/jobs` as any, search: { tab: 'maintenance' } as any });
  };
  ...
  return (
    <AppShell className="bg-muted/10">
      <div className="max-w-2xl mx-auto pb-24">
        {/* Header Section */}
        <div className="mb-6">
          <Link to={`${prefix}/tech/jobs` as any} search={{ tab: 'maintenance' } as any} className="flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground mb-4 w-fit">
        ...
        {/* USED PARTS SUMMARY */}
        {usedParts.length > 0 && (
          <Card className="mt-6 p-6 border-none shadow-lg rounded-3xl bg-background/50 backdrop-blur-md">
