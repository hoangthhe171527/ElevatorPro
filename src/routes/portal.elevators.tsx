...
import { Building2, QrCode, MapPin, Search } from "lucide-react";
...
function PortalElevators() {
  ...
  const filtered = mockElevators.filter(e => e.customerId === CUSTOMER_ID).filter(e =>
    !search || e.code.toLowerCase().includes(search.toLowerCase()) || e.building.toLowerCase().includes(search.toLowerCase())
  );
  const paged = filtered.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);

  return (
    <AppShell>
      <PageHeader title="Thang máy của bạn" description={`${filtered.length} thang đang được quản lý`} />

      <Card className="overflow-hidden">
        <div className="p-4 border-b">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
            <Input placeholder="Tìm thang máy..." className="pl-9" value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} />
          </div>
        </div>
          ...
          {paged.map(e => (
            <Card key={e.id} className="p-4">
              ...
              <div className="space-y-1 text-xs text-muted-foreground">
                <div className="flex items-start gap-1"><MapPin className="h-3 w-3 mt-0.5 shrink-0" /> {e.building}</div>
                <div>{e.floors} tầng · BT gần nhất {formatDate(e.lastMaintenance)}</div>
                <div>BT tiếp: <span className="font-medium text-foreground">{formatDate(e.nextMaintenance)}</span></div>
              </div>
              <div className="mt-3 pt-3 border-t flex gap-2">
                <Link to="/qr/$elevatorId" params={{ elevatorId: e.id }} className="flex-1">
                  <Button variant="outline" size="sm" className="w-full"><QrCode className="h-3.5 w-3.5 mr-1" /> Xem mã QR</Button>
                </Link>
              </div>
            </Card>
          ))}
