    ...
    <AppShell>
      ...
      <div className="grid grid-cols-1 gap-4">
         {warrantyJobs.map(job => {
           ...
           return (
             <Card key={job.id} className="rounded-2xl border-none shadow-sm hover:shadow-md transition-all overflow-hidden">
                <CardContent className="p-0">
                   <div className="p-4 bg-emerald-50 border-b flex justify-between items-center">
                      <div className="flex items-center gap-2">
                         <ShieldCheck className="h-4 w-4 text-emerald-600" />
                         <span className="text-[10px] font-black uppercase text-emerald-700 tracking-widest">Gói bảo hành Gold</span>
                      </div>
                      <Badge className={cn(
                        "px-2 py-0.5 rounded-lg font-black text-[9px] uppercase",
                        job.status === 'completed' ? "bg-emerald-500 text-white" : "bg-white text-emerald-600 border-emerald-200"
                      )}>
