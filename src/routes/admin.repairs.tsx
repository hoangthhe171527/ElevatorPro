      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 mb-8">
         <Card className="rounded-[2rem] border-none shadow-xl shadow-slate-200/50 p-6 bg-orange-500 text-white">
            <div className="flex items-center gap-4">
               ...
               <div>
                  <div className="text-3xl font-black">{repairJobs.filter(j => j.status !== 'completed').length}</div>
         ...
         <Card className="rounded-[2rem] border-none shadow-xl shadow-slate-200/50 p-6 bg-slate-900 text-white">
            <div className="flex items-center gap-4">
               ...
               <div>
                  <div className="text-3xl font-black">{repairJobs.filter(j => j.status === 'payment_pending').length}</div>
         ...
         <Card className="rounded-[2rem] border-none shadow-xl shadow-slate-200/50 p-6 bg-white">
            <div className="flex items-center gap-4">
               ...
               <div>
                  ...
                  <div className="text-[10px] font-black uppercase text-slate-400">Sự cố khẩn cấp (HOTLINE)</div>
               </div>
            </div>
         </Card>
         ...
         <Card className="rounded-[2rem] border-none shadow-xl shadow-slate-200/50 p-6 bg-white">
            <div className="flex items-center gap-4">
               ...
               <div>
                  ...
                  <div className="text-[10px] font-black uppercase text-slate-400">Thời gian phản hồi TB</div>
               </div>
            </div>
         </Card>
      </div>
                  ...
                  <CardContent className="p-6 flex items-center justify-between">
                     <div className="flex gap-6 items-center">
                        <div className={cn(
                          ...
                          job.status === 'payment_pending' ? "bg-amber-100 text-amber-600" : "bg-slate-100 text-slate-400"
                        )}>
                           {job.status === 'payment_pending' ? <Package className="h-6 w-6" /> : <Wrench className="h-6 w-6" />}
                        </div>
                        <div>
                           <div className="flex items-center gap-3 mb-1">
                              ...
                              <Badge className={cn(
                                ...
                                job.status === 'payment_pending' ? "bg-amber-500 text-white" : "bg-slate-200 text-slate-600"
                              )}>
                                 {job.status === 'payment_pending' ? 'Chờ duyệt báo giá' : job.status}
                              </Badge>
                              {job.isHotline && <Badge className="bg-red-500 text-white font-black text-[9px] uppercase animate-pulse">Hotline</Badge>}
                           </div>
                           <div className="text-sm font-bold text-slate-500 flex items-center gap-4">
                              ...
                              {job.repairQuote && <span className="text-primary font-black ml-2">Dự chi: {formatVND(job.repairQuote.total)}</span>}
                           </div>
                        </div>
                     </div>
                     <div className="flex items-center gap-4">
                        <Link to="/admin/jobs/$jobId" params={{ jobId: job.id }}>
                           <Button variant="outline" className="rounded-xl font-black text-[10px] uppercase gap-2 hover:bg-slate-900 hover:text-white transition-all">
