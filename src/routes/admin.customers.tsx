...
import { Plus, Search, Building2, User, Phone, Mail, MapPin, FileText, ChevronRight } from "lucide-react";
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        actions={<Button><Plus className="h-4 w-4 mr-1.5" /> Thêm khách hàng</Button>}
      />
      ...
      <Card className="overflow-hidden">
        <div className="flex flex-col sm:flex-row gap-3 p-4 border-b">
          <div className="flex-1 relative">
            ...
            <Input placeholder="Tìm khách hàng..." className="pl-9" value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} />
          </div>
              ...
              <div key={c.id} className="p-4 hover:bg-muted/30 transition-colors">
                <div className="flex flex-col md:flex-row md:items-center gap-4">
                  <div className="flex items-start gap-3 flex-1 min-w-0">
                    <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-lg ${c.type === "business" ? "bg-primary/10 text-primary" : "bg-info/10 text-info"}`}>
                      {c.type === "business" ? <Building2 className="h-5 w-5" /> : <User className="h-5 w-5" />}
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <h3 className="font-semibold truncate">{c.name}</h3>
                        <Badge variant="outline" className="text-[10px]">{c.type === "business" ? "Doanh nghiệp" : "Cá nhân"}</Badge>
                      </div>
                      <div className="mt-1 text-xs text-muted-foreground">Người liên hệ: {c.contactPerson}</div>
