  ...
  return (
    <AppShell>
      ...
      <PageHeader 
        ...
        actions={
          <Badge variant="outline" className="h-8 px-4 bg-primary/5 text-primary border-primary/20 font-bold">
            GIAI ĐOẠN LẮP ĐẶT
          </Badge>
        }
      />
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-4">
          <Card className="p-5 bg-gradient-to-br from-primary/5 to-transparent border-primary/10">
            <h3 className="font-bold text-sm uppercase tracking-widest text-muted-foreground mb-4">Quy trình 8 bước thi công</h3>
            <div className="space-y-3">
              {INSTALL_STAGES_TEMPLATE.map((stage, idx) => {
                const job = projectJobs.find(j => j.title.includes(stage.label) || j.code.includes(`INSTALL-${idx + 1}`));
                    ...
                    `}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2 mb-1">
                          <h4 className={`font-bold transition-colors ${isCurrent ? "text-primary text-base" : "text-sm"}`}>
                        ...
                        {job ? (
                          <div className="mt-2 flex items-center justify-between">
                            ...
                            <Link to={`${prefix}/tech/jobs/$jobId` as any} params={{ jobId: job.id } as any}>
                              <Button size="sm" variant={isCurrent ? "default" : "outline"} className="h-7 text-[10px] font-bold uppercase tracking-tight">
                                {isCompleted ? "Xem lại BC" : isCurrent ? "Thực hiện ngay" : "Chi tiết"}
                                <ChevronRight className="h-3 w-3 ml-1" />
                              </Button>
                            </Link>
                          </div>
                        ) : (
                          <div className="mt-1 text-[10px] text-muted-foreground italic">Chưa có lịch làm việc cho giai đoạn này</div>
                        )}
                      </div>
                    </div>
                  </div>
          ...
          <Card className="p-5">
            ...
            {cus && (
              <div className="space-y-3 text-sm">
                <div>
                  <label className="text-[10px] font-bold uppercase text-muted-foreground block mb-0.5">Khách hàng</label>
                  <p className="font-bold">{cus.name}</p>
                </div>
                <div>
                  <label className="text-[10px] font-bold uppercase text-muted-foreground block mb-0.5">Địa chỉ thi công</label>
                  <p className="text-muted-foreground leading-relaxed">{project.address}</p>
                </div>
                <div>
                  <label className="text-[10px] font-bold uppercase text-muted-foreground block mb-0.5">Ngày khởi công</label>
            ...
            <div className="p-3 rounded-lg bg-orange-50 border border-orange-100 text-[11px] text-orange-700 leading-relaxed italic">
              * Chú ý: Kỹ thuật viên thực hiện đúng trình tự các bước. Giai đoạn sau chỉ được phép thực hiện khi giai đoạn trước đã được xác nhận hoàn thành/nghiệm thu.
            </div>
          </Card>
        </div>
      </div>
