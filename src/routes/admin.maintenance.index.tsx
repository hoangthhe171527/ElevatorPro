...
function MaintenanceOversightPage() {
  ...
  const enrichedContracts = useMemo(() => {
    return maintenanceContracts.map(c => {
      ...
      const latestJob = [...mockJobs]
        .filter(j => j.contractId === c.id && (j.type === 'maintenance' || j.type === 'warranty'))
  ...
  const stats = {
    ...
    activeWork: enrichedContracts.filter(c => c.latestJob && c.latestJob.status === 'in_progress').length,
    waitingMaterials: enrichedContracts.filter(c => c.latestJob && c.latestJob.status === 'waiting_for_materials').length,
  };
  ...
  return (
    ...
    <AppShell>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-6">
        ...
        />
        <StatsCard title="Đang thực hiện" value={stats.activeWork} icon={Activity} color="indigo" />
        <StatsCard title="Chờ vật tư" value={stats.waitingMaterials} icon={ArrowRightLeft} color="orange" />
      </div>
      ...
      <div className="grid gap-4">
        {enrichedContracts.map(c => (
          <Card key={c.id} className={`overflow-hidden border-l-4 transition-all hover:shadow-md ${c.isExpiringSoon ? 'border-l-rose-500' : c.isExpired ? 'border-l-destructive' : 'border-l-primary'}`}>
            <div className="p-5 flex flex-col lg:flex-row gap-6 lg:items-center">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  {c.isExpiringSoon && (
                    <Badge className="bg-rose-500 text-white animate-pulse">Sắp hết hạn ({c.daysToExpiry} ngày)</Badge>
                  )}
                  {c.isExpired && (
                ...
                <div className="text-sm text-muted-foreground flex items-center gap-2">
                   <span className="font-medium text-foreground">{c.elevatorCode}</span> • {c.elevatorModel}
                </div>
              </div>
              ...
              <div className="lg:w-64">
                ...
                <Progress value={c.progress} className={`h-2 ${c.progress === 100 ? "[&>div]:bg-success" : ""}`} />
                <div className="mt-2 text-[10px] flex items-center justify-between text-muted-foreground">
                   <span>Trạng thái: <strong>{c.latestJob?.status || "N/A"}</strong></span>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-1 gap-2 lg:text-right min-w-[150px]">
                <div>
                   <div className="text-[10px] text-muted-foreground uppercase font-bold">Ngày hết hạn</div>
                   <div className={`text-sm font-bold ${c.isExpiringSoon ? 'text-rose-600' : ''}`}>{formatDate(c.endDate)}</div>
                </div>
                <div className="lg:mt-1">
                   <div className="text-[10px] text-muted-foreground uppercase font-bold">Giá trị HĐ</div>
              ...
              <div className="flex gap-2 lg:flex-col shrink-0">
                <Button size="sm" variant="outline" className="flex-1 lg:w-32" onClick={() => toast.info(`Liên hệ khách hàng: ${c.customerName}`)}>
        ...
        {enrichedContracts.length === 0 && (
          <div className="py-20 text-center border-2 border-dashed rounded-3xl bg-muted/5">
             ...
             <h3 className="font-bold text-lg text-muted-foreground">Không tìm thấy hợp đồng bảo trì nào</h3>
...
function StatsCard({ title, value, icon: Icon, color, highlight }: any) {
  ...
  return (
    <Card className={`p-4 border transition-all ${highlight ? 'ring-2 ring-rose-500 ring-offset-2' : ''}`}>
      <div className="flex justify-between items-start">
        <div>
          <div className="text-xs font-bold text-muted-foreground uppercase mb-1 tracking-wider">{title}</div>
