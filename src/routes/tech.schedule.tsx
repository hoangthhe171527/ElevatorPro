      ...
      <div className="space-y-6 pb-20">
          ...
          <div key={day} className="space-y-3 px-1">
            <div className="flex items-center justify-between px-2">
               <div className="flex flex-col">
                  <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">{formatDate(day)}</span>
                  <span className="text-lg font-black text-slate-800 capitalize">{getDayName(day)}</span>
               </div>
            ...
            <div className="grid gap-3">
              {jobs.map((j) => {
                ...
                return (
                  <Link
                    key={j.id}
                    to={(j.type === 'inspection' ? `${prefix}/tech/survey/$jobId` : `${prefix}/tech/jobs/$jobId`) as any}
                  ...
                  >
                    <Card className="overflow-hidden border-none shadow-sm hover:shadow-md transition-all active:scale-[0.98]">
                       <div className="flex">
                          {/* Type Indicator Bar */}
                          <div className={`w-1.5 ${
                            j.type === 'maintenance' ? 'bg-blue-500' : 
                            j.type === 'install' ? 'bg-orange-500' : 'bg-destructive'
                          }`} />
                             ...
                             <div className="flex items-start justify-between gap-4">
                                <div className="min-w-0">
                                   <div className="flex items-center gap-2 mb-1">
                                      <span className="font-black text-xs uppercase text-slate-800 truncate">{j.title}</span>
                                      {j.priority === 'urgent' && (
                                        <div className="h-1.5 w-1.5 rounded-full bg-destructive animate-ping" />
                                      )}
                                   </div>
                                   <div className="flex items-center gap-1.5 text-[10px] text-slate-500 font-bold uppercase">
                                      <Clock className="h-3 w-3" /> {formatDateTime(j.scheduledFor).split(' ')[1]}
                                      <span className="mx-1 opacity-30">|</span>
                                      <MapPin className="h-3 w-3" /> {cus?.name}
                                   </div>
                                </div>
                                <div className="text-right shrink-0">
                                   <StatusBadge variant={jobStatusVariant[j.status]} className="text-[8px] px-1.5 py-0 uppercase font-black">
        ...
        {Object.keys(grouped).length === 0 && (
          <Card className="p-12 text-center border-dashed bg-transparent">
             ...
             <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">Không có lịch làm việc</p>
          </Card>
        )}
      </div>
    </AppShell>
