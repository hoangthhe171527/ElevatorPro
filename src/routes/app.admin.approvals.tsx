  ...
  return (
    <AppShell>
      <PageHeader
        ...
        description="Quản lý và xét duyệt các yêu cầu"
      />
      ...
      <div className="flex flex-col gap-4 pb-8">
        {isSmall ? (
          ...
          <div className="py-20 text-center">
             ...
             <p className="text-slate-400 font-bold uppercase text-[11px] tracking-widest">Trống. Không có đơn chờ</p>
          </div>
        ) : (
          paged.map((req) => {
            ...
            return (
              <div key={req.id} className="relative bg-white p-5 rounded-[24px] shadow-[0_2px_10px_rgba(0,0,0,0.03)] border border-slate-100">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex gap-3">
                    ...
                    <div>
                      <h3 className="font-black text-[15px] text-slate-800 leading-tight mb-0.5">{TYPE_LABELS[req.type as keyof typeof TYPE_LABELS]}</h3>
                      <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Đơn #{req.id}</div>
                    </div>
                  </div>
                  <Badge variant={req.status === "pending" ? "outline" : req.status === "approved" ? "default" : "destructive"} className="text-[9px] uppercase font-black px-2 py-0.5 rounded-full">
                ...
                <div className="mb-4">
                  ...
                  <p className="text-[12px] text-slate-500 line-clamp-2 leading-relaxed">{req.description}</p>
                  {req.amount && (
                    <div className="mt-3 text-lg font-black text-rose-500">{formatVND(req.amount)}</div>
                  )}
                  {project && (
                ...
                <div className="flex items-center justify-between pt-4 border-t border-slate-50">
                  ...
                  {req.status === "pending" && (
                    <div className="flex gap-2">
                      <Button size="sm" variant="outline" className="h-9 rounded-xl text-[10px] uppercase font-black border-slate-200 text-slate-600" onClick={() => handleAction(req.id, "rejected")}>Bác bỏ</Button>
                      <Button size="sm" className="h-9 rounded-xl text-[10px] uppercase font-black bg-emerald-500 hover:bg-emerald-600" onClick={() => handleAction(req.id, "approved")}>Duyệt</Button>
                    </div>
                  )}
                </div>
              </div>
