...
import {
  mockJobs, mockContracts, mockCustomers, mockElevators, mockLeads,
  formatVND, formatDate, formatDateTime, getCustomer
} from "@/lib/mock-data";
import { Briefcase, Users, FileText, AlertTriangle, Building2, TrendingUp, Calendar, ArrowUpRight } from "lucide-react";
  ...
  return (
    <AppShell>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-6">
        ...
        <StatCard label="Thang đến hạn BT" value={overdueElevators} icon={AlertTriangle} accent="destructive" hint="Cần lên lịch ngay" />
      </div>

      <div className="grid gap-4 lg:grid-cols-3 mb-6">
        <StatCard label="Tổng khách hàng" value={mockCustomers.length} icon={Users} accent="primary" />
        <StatCard label="Tổng thang máy quản lý" value={mockElevators.length} icon={Building2} accent="primary" />
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <Card className="lg:col-span-2 p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/admin/jobs"><Button variant="ghost" size="sm">Tất cả</Button></Link>
          </div>
          <div className="divide-y">
            {upcomingJobs.map(j => {
              const cus = getCustomer(j.customerId);
              return (
                <Link key={j.id} to="/admin/jobs/$jobId" params={{ jobId: j.id }} className="flex items-center gap-3 py-3 hover:bg-muted/50 -mx-2 px-2 rounded">
        ...
        <Card className="p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/admin/contracts"><Button variant="ghost" size="sm">Tất cả</Button></Link>
          </div>
          <div className="space-y-3">
            {recentContracts.map(c => {
              const cus = getCustomer(c.customerId);
              return (
                <Link key={c.id} to="/admin/contracts" className="block p-3 rounded-lg border hover:bg-muted/50 transition-colors">
      ...
      <Card className="mt-6 p-5">
        ...
        <div className="grid gap-3 md:grid-cols-3">
          {[
            ...
            const Icon = a.icon;
            return (
              <Link key={a.label} to={a.href} className="flex items-center gap-3 p-3 rounded-lg border hover:bg-muted/50">
