      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 mb-8">
         ...
         <Card className="rounded-[2rem] border-none shadow-xl shadow-slate-200/50 p-6 bg-slate-900 text-white">
            <div className="flex items-center gap-4">
               ...
               <div>
                  <div className="text-3xl font-black">{warrantyJobs.filter(j => j.status === 'pending').length}</div>
         ...
         <Card className="rounded-[2rem] border-none shadow-xl shadow-slate-200/50 p-6 bg-white">
            <div className="flex items-center gap-4">
               ...
               <div>
                  ...
                  <div className="text-[10px] font-black uppercase text-slate-400">Sắp hết hạn (30 ngày)</div>
               </div>
            </div>
         </Card>
         ...
         <Card className="rounded-[2rem] border-none shadow-xl shadow-slate-200/50 p-6 bg-white">
            <div className="flex items-center gap-4">
               ...
               <div>
                  ...
                  <div className="text-[10px] font-black uppercase text-slate-400">Tỉ lệ đạt chuẩn vận hành</div>
               </div>
            </div>
         </Card>
      </div>
      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
           {warrantyJobs.map(job => {
             ...
             return (
               <Card key={job.id} className="rounded-[2rem] border-none shadow-lg shadow-slate-200/40 hover:shadow-xl transition-all overflow-hidden group">
                  <CardContent className="p-0">
                     <div className="p-6 bg-slate-50 border-b flex justify-between items-center">
                        ...
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{job.code}</span>
                     </div>
                     <div className="p-6 space-y-4">
                        <div>
                           <h4 className="font-black text-slate-900 text-lg group-hover:text-primary transition-colors">{job.title}</h4>
                        ...
                        <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                           <div className="flex items-center gap-2">
                              ...
                              <span className="text-sm font-bold text-slate-700">{formatDate(job.scheduledFor)}</span>
                           </div>
                           <Link to="/admin/jobs/$jobId" params={{ jobId: job.id }}>
                              <Button size="sm" variant="ghost" className="rounded-xl font-black text-[10px] uppercase gap-2 hover:bg-primary hover:text-white">
