  ...
  return (
    <AppShell>
      <PageHeader
        ...
        actions={<Button><Plus className="h-4 w-4 mr-1.5" /> Thêm thang</Button>}
      />
      ...
      <Card className="overflow-hidden">
        <div className="flex flex-col sm:flex-row gap-3 p-4 border-b">
          <div className="flex-1 relative">
            ...
            <Input placeholder="Tìm mã thang, khách hàng..." className="pl-9" value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} />
          </div>
          <Select value={statusFilter} onValueChange={(v) => { setStatusFilter(v); setPage(1); }}>
            <SelectTrigger className="w-full sm:w-44"><SelectValue /></SelectTrigger>
            <SelectContent>
              <SelectItem value="all">Tất cả trạng thái</SelectItem>
              {Object.entries(elevatorStatusLabel).map(([k, v]) => <SelectItem key={k} value={k}>{v}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
            ...
            return (
              <Card key={e.id} className="p-4 hover:shadow-elevated transition-shadow">
                ...
                <div className="space-y-1.5 text-xs">
                  <div className="text-muted-foreground">Khách: <span className="text-foreground font-medium">{cus?.name}</span></div>
                  <div className="text-muted-foreground flex items-start gap-1"><MapPin className="h-3 w-3 mt-0.5 shrink-0" /> {e.building}, {e.address}</div>
                  <div className="text-muted-foreground">{e.floors} tầng · BT gần nhất {formatDate(e.lastMaintenance)}</div>
                  <div className="text-muted-foreground">BT tiếp theo: <span className="text-foreground font-medium">{formatDate(e.nextMaintenance)}</span></div>
                </div>
                <div className="mt-3 pt-3 border-t flex gap-2">
                  <Link to="/qr/$elevatorId" params={{ elevatorId: e.id }} className="flex-1">
                    <Button variant="outline" size="sm" className="w-full"><QrCode className="h-3.5 w-3.5 mr-1" /> Xem QR</Button>
                  </Link>
                  <Button size="sm" className="flex-1">Chi tiết</Button>
                </div>
              </Card>
            );
          })}
