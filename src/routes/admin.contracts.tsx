...
import { mockContracts, formatVND, formatDate, getCustomer } from "@/lib/mock-data";
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        actions={<Button><Plus className="h-4 w-4 mr-1.5" /> Tạo hợp đồng</Button>}
      />
      ...
      <div className="grid gap-4 sm:grid-cols-3 mb-6">
        ...
        <Card className="p-4">
          ...
          <div className="mt-1 text-xl font-bold text-warning-foreground">{formatVND(totalValue - totalPaid)}</div>
        </Card>
      </div>
      ...
      <Card className="overflow-hidden">
        <div className="flex flex-col sm:flex-row gap-3 p-4 border-b">
          <div className="flex-1 relative">
            ...
            <Input placeholder="Tìm mã HĐ hoặc khách hàng..." className="pl-9" value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} />
          </div>
          ...
          <Select value={statusFilter} onValueChange={(v) => { setStatusFilter(v); setPage(1); }}>
            ...
            <SelectContent>
              ...
              {Object.entries(contractStatusLabel).map(([k, v]) => <SelectItem key={k} value={k}>{v}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
                ...
                <div className="flex flex-col lg:flex-row gap-4">
                  <div className="flex items-start gap-3 flex-1 min-w-0">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-2 text-xs text-muted-foreground italic">{c.items.join(" · ")}</div>
                    </div>
                  </div>
