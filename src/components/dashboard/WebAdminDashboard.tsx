      ...
      {isCEO && expiringWarranties?.length > 0 && (
        <div className="mb-6 p-4 rounded-3xl bg-orange-50 border-2 border-orange-100 flex flex-col md:flex-row items-center justify-between gap-4 animate-in fade-in slide-in-from-top-4 duration-1000 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h4 className="font-black text-orange-800 uppercase tracking-tight text-sm">
                Nhắc nhở: {expiringWarranties.length} thang sắp hết hạn bảo hành
              </h4>
              <p className="text-xs text-orange-700/70 font-medium">
                Cần liên hệ khách hàng để tư vấn chuyển sang hợp đồng bảo trì định kỳ.
              </p>
            </div>
          </div>
      ...
      {isCEO ? (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 mb-6">
            <div
              ...
              onClick={() => navigate({ to: "/admin/jobs", search: { tab: "pending" } })}
            >
              <StatCard
            ...
            <div
              className="cursor-pointer"
              onClick={() => navigate({ to: "/admin/jobs", search: { priority: "urgent" } })}
            >
              <StatCard
                label="Sự cố khẩn cấp"
                value={
                  tenantJobs.filter((j) => j.priority === "urgent" && j.status !== "completed")
                    .length
                }
                icon={AlertTriangle}
                accent="destructive"
            ...
            <Card className="p-6 shadow-xl shadow-slate-200/50 lg:col-span-2 border-none bg-white">
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <h3 className="font-black text-slate-800 tracking-tight uppercase truncate text-sm">
              ...
              <div className="grid gap-4 sm:grid-cols-3 md:gap-6">
                <div className="p-4 rounded-3xl bg-slate-50 border border-slate-100 flex flex-col items-center justify-center text-center">
                  ...
                  <div className="font-black tracking-tighter text-slate-800 text-4xl">
                    {monthlyJobs.length}
                  </div>
                </div>
                ...
                <div className="p-4 rounded-3xl bg-emerald-50/50 border border-emerald-100/50 flex flex-col items-center justify-center text-center">
                  ...
                  <div className="font-black tracking-tighter text-emerald-600 text-4xl">
                    {completedJobsThisMonth}
                  </div>
                </div>
                ...
                <div className="p-4 rounded-3xl bg-warning/5 border border-warning/10 flex flex-col items-center justify-center text-center">
                  ...
                  <div className="font-black tracking-tighter text-warning-foreground text-4xl">
                    {pendingJobsThisMonth}
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                {techPerformance.map((tech, idx) => {
                  ...
                  return (
                    <div
                      key={tech.id}
                      className={`flex items-center justify-between p-3 rounded-2xl bg-slate-50/50 border ${isRecommended ? "border-primary/40 bg-primary/[0.02]" : "border-transparent"} hover:border-primary/20 hover:bg-white transition-all group`}
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={`h-8 w-8 rounded-full border shadow-sm flex items-center justify-center text-[10px] font-black ${isRecommended ? "bg-primary text-white" : "bg-white text-slate-400"}`}
                        >
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-black uppercase tracking-tight text-slate-800">
                Tiến độ Dự án lắp đặt trọn gói
              </h3>
              <Link to="/admin/projects" className="text-xs text-primary font-bold hover:underline">
                Tất cả dự án →
              </Link>
            </div>
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {mockProjects
                .filter((p) => p.status === "in_progress" && p.tenantId === activeTenantId)
                .slice(0, 3)
                .map((p) => {
                  const projJobs = mockJobs.filter((j) => j.projectId === p.id);
                  const doneJobs = projJobs.filter((j) => j.status === "completed").length;
                  const progress =
                    projJobs.length > 0 ? Math.round((doneJobs / projJobs.length) * 100) : 0;
                    ...
                    <Link key={p.id} to="/admin/projects/$projectId" params={{ projectId: p.id }}>
                      <Card className="p-5 hover:shadow-xl transition-all group border-none shadow-md bg-white">
                        <div className="flex justify-between items-start mb-3">
                          <h4 className="font-black text-slate-800 group-hover:text-primary transition-colors line-clamp-1">
                            {p.name}
                          </h4>
                          <Badge
                            variant="outline"
                        ...
                        <div className="mt-4 pt-4 border-t border-slate-50 flex items-center justify-between">
                          <div className="flex -space-x-2">
                            {Array.from({ length: 3 }).map((_, i) => (
                              <div
                                key={i}
                                className="h-6 w-6 rounded-full border-2 border-white bg-slate-200 flex items-center justify-center text-[8px] font-black text-slate-500 uppercase"
                              >
                                NV
                              </div>
                            ))}
              ...
              <div className="grid gap-4 md:grid-cols-3">
                {[
                  ...
                  },
                  {
                    ...
                    count: mockContracts.filter((c) => c.accountantVerified && !c.ceoVerified)
                      .length,
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-[9px] opacity-70 font-bold mt-1 line-clamp-1">
                        {a.desc}
                      </div>
                    </Link>
                  );
              ...
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">
                      Vận hành hiện trường thời gian thực
                    </p>
                  </div>
                </div>
                <Link to="/admin/jobs">
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="font-black text-[10px] uppercase text-slate-800 truncate">
                                  {j.title}
                                </div>
                ...
                {recentContracts.map((c) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-[10px] font-bold text-slate-400 uppercase truncate mb-3">
                        {cus?.name}
                      </div>
        ...
        <div className="grid gap-6 lg:grid-cols-3 mb-6">
          <Card className="lg:col-span-2 p-6 border-none shadow-sm bg-white">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-black uppercase tracking-tight text-slate-800">
                Việc bạn cần xử lý hôm nay
              </h3>
              <Badge variant="secondary" className="text-[10px]">
                {personalQueue.length} mục
              </Badge>
            </div>
            <div className="space-y-2">
