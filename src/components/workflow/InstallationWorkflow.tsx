                  ...
                  <div key={stage} className="flex flex-col items-center gap-3 w-12 sm:w-20">
                    <div
                      className={`
                        ...
                        ${
                          isPast
                            ? "bg-primary text-primary-foreground scale-90 opacity-80"
                            : isCurrent
                              ? "bg-background border-2 border-primary text-primary ring-4 ring-primary/10 scale-110 shadow-lg"
                    ...
                    <div className="text-center">
                      <div
                        className={`text-[10px] font-bold uppercase tracking-tight leading-tight ${isCurrent ? "text-primary" : "text-muted-foreground"}`}
                      >
                        GĐ {idx + 1}
                      </div>
                      <div
                        className={`hidden sm:block text-[11px] font-medium mt-0.5 ${isCurrent ? "text-foreground font-bold" : "text-muted-foreground"}`}
                      >
                        {label}
                      </div>
                    </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <h3 className="text-lg font-semibold flex items-center gap-2">
                Giai đoạn:{" "}
                <span className="text-primary font-bold">{PROJECT_STAGE_LABELS[currentStage]}</span>
              </h3>
            ...
            <div className="flex items-center gap-2">
              <Badge
                variant="outline"
                className={`${allTasksDone ? "bg-success/10 text-success" : "bg-blue-50"} animate-none`}
              >
              ...
              {currentStage === "contract" && (
                <Button
                  ...
                  onClick={() =>
                    toast.success("Đã gửi bản thảo hợp đồng qua Zalo và Email cho khách!")
                  }
                >
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`flex h-6 w-6 items-center justify-center rounded-full ${isDone ? "bg-success text-white" : "bg-muted text-muted-foreground"}`}
                      >
                        {isDone ? (
                    ...
                    {!hasPermission && isDone && (
                      <Badge
                        variant="outline"
                        className="h-7 text-[9px] bg-success/5 text-success border-success/20"
                      >
                        Đã xong
                      </Badge>
                    )}
      ...
      <ConfirmationDialog
        ...
        title={
          confirmAction === "advance"
            ...
            : "CẢNH BÁO: Phá vỡ quy trình (Break Flow)"
        }
        description={
          confirmAction === "advance"
