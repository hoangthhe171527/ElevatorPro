      ...
      <PageHeader
        ...
        actions={
          <Button
            size="sm"
            variant="outline"
            className="h-8 rounded-full px-3 text-[10px] font-black uppercase text-primary border-primary bg-primary/5"
            onClick={() => toast.success("Đã xuất báo cáo gửi tới Zalo Chủ tịch.")}
          >
            Chia sẻ Zalo
          </Button>
        }
      ...
      <div className="flex bg-slate-100 p-1 rounded-2xl mb-6 shadow-inner mx-1">
        <button
          className={cn(
            ...
            activeTab === "overview" ? "bg-white text-primary shadow-sm" : "text-slate-400",
          )}
        ...
        <button
          className={cn(
            "flex-1 py-2 text-[11px] font-black uppercase tracking-widest rounded-xl transition-all",
            activeTab === "charts" ? "bg-white text-primary shadow-sm" : "text-slate-400",
          )}
      ...
      {activeTab === "overview" ? (
        <div className="space-y-4 pb-8">
          ...
          <div className="bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-[24px] p-6 text-white shadow-xl shadow-emerald-500/20 relative overflow-hidden">
            ...
            <div className="inline-flex items-center bg-white/20 px-2 py-0.5 rounded-full text-[10px] font-bold">
          ...
          <div className="flex overflow-x-auto hide-scrollbar gap-2 snap-x px-1">
            {months.map((m) => (
              <button
                ...
                onClick={() => setPeriod(m)}
                className={cn(
                  "px-4 py-2 rounded-full text-[11px] font-black uppercase tracking-widest snap-center whitespace-nowrap transition-all",
                  period === m
                ...
                return (
                  <div key={m} className="flex-1 flex flex-col items-center gap-1.5">
                    <div className="w-full flex flex-col justify-end" style={{ height: "120px" }}>
                      ...
                      >
                        {isSelected && (
                          <div className="absolute -top-6 left-1/2 -translate-x-1/2 text-[10px] font-black text-primary">
                            {monthlyJobs[i]}
                          </div>
                        )}
                      </div>
                    ...
                    <div
                      className={`text-[9px] font-black ${isSelected ? "text-primary" : "text-slate-400 opacity-50"}`}
                    >
                      {m.split("/")[0]}
                    </div>
                  </div>
                ...
                return (
                  <div key={m} className="flex-1 flex flex-col items-center gap-1.5">
                    <div className="w-full flex flex-col justify-end" style={{ height: "120px" }}>
                      ...
                      >
                        {isSelected && (
                          <div className="absolute -top-6 left-1/2 -translate-x-1/2 text-[10px] font-black text-emerald-600">
                            {monthlyRevenue[i]}
                          </div>
                        )}
                      </div>
                    ...
                    <div
                      className={`text-[9px] font-black ${isSelected ? "text-emerald-500" : "text-slate-400 opacity-50"}`}
                    >
                      {m.split("/")[0]}
                    </div>
                  </div>
