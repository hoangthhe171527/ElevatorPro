...
export function WebLeads() {
  ...
  const filtered = leads.filter((l) => {
    ...
    return matchTenant && matchOwner && matchSearch && matchStatus && matchType;
  });
        ...
        <Card className="shadow-none border-dashed bg-muted/20">
          <CardContent className="pt-6">
            ...
            <div className="mt-2 text-3xl font-black text-success">
              {Math.round(
                (initialLeads.filter((l) => l.status === "signed").length / initialLeads.length) *
                  100,
              )}
              %
            </div>
      ...
      <div className="grid gap-6">
        {paged.map((l) => {
          ...
          return (
            <Card
              key={l.id}
              className="overflow-hidden border-none shadow-sm hover:shadow-md transition-all duration-300 rounded-2xl"
            >
                ...
                <div className="p-6 flex-1 flex flex-col">
                    ...
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex flex-col">
                        ...
                        <StatusBadge
                          ...
                          className="mt-1 w-fit uppercase text-[9px] font-black px-2 py-0"
                        >
                    ...
                    <div className="flex items-center justify-between relative px-2 mt-6">
                      {PIPELINE_STAGES.map((s, idx) => (
                        <div key={s.key} className="flex flex-col items-center gap-2 z-10">
                          <div
                            className={`h-4 w-4 rounded-full ring-4 ring-white transition-all duration-700 ${idx <= statusIdx ? "bg-primary shadow-lg shadow-primary/30" : "bg-slate-200"}`}
                          />
                          <span
                            className={`text-[9px] font-black uppercase tracking-tighter ${idx === statusIdx ? "text-primary" : "text-slate-400 opacity-60"}`}
                          >
                            {s.label}
                          </span>
                        </div>
                  ...
                  <div className="flex flex-wrap items-center gap-3 mt-auto pt-4 border-t border-slate-50">
                    {l.status === "new" && (
                      <Button
                        size="sm"
                        className="bg-primary hover:bg-primary/90 h-10 px-4 rounded-xl font-bold gap-2"
                        onClick={() => setSurveyLead(l)}
                        disabled={!canManageLeadFlow}
                        title={
                          !canManageLeadFlow
                            ? "Chỉ Sales Admin/PM/CEO được giao khảo sát"
                            : undefined
                        }
                      >
                    ...
                    {l.status === "surveying" && (
                      <div className="flex flex-col gap-2 w-full">
                        <div className="flex items-center gap-2 text-xs font-bold text-amber-600 bg-amber-50 p-3 rounded-xl border border-amber-100">
                          <Clock className="h-4 w-4 animate-spin" /> Đang chờ kỹ thuật viên khảo sát
                          thực tế...
                        </div>
                        ...
                        <Button
                          size="sm"
                          variant="ghost"
                          className="text-[10px] text-slate-400 hover:text-primary self-start"
                    ...
                    {l.status === "surveyed" && (
                      <Button
                        size="sm"
                        className="bg-orange-500 hover:bg-orange-600 h-10 px-4 rounded-xl font-bold gap-2 shadow-lg shadow-orange-200"
                    ...
                    {l.status === "quoted" && (
                      <div className="flex gap-3 w-full">
                        <Button
                          className="flex-1 h-10 bg-indigo-600 hover:bg-indigo-700 rounded-xl font-bold gap-2 shadow-lg shadow-indigo-100"
                        ...
                        <Button
                          variant="outline"
                          className="h-10 rounded-xl font-bold text-slate-400 border-slate-200"
                    ...
                    {l.status === "signed" && (
                      <div className="flex items-center gap-2 text-success bg-success/10 px-4 py-2 rounded-xl border border-success/20 w-full animate-in zoom-in duration-300">
                        ...
                        <span className="text-sm font-black uppercase">
                          Đã chốt - Sẵn sàng triển khai
                        </span>
                        <Button
                          size="sm"
                          className="ml-auto bg-success hover:bg-success/90 rounded-lg text-[10px]"
                          onClick={() => setConfirmConvert(l)}
                          disabled={!canManageLeadFlow}
                          title={
                            !canManageLeadFlow
                              ? "Chỉ Sales Admin/PM/CEO được kích hoạt dự án"
                              : undefined
                          }
                        >
                          KÍCH HOẠT DỰ ÁN
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem
                            ...
                            onClick={() => {
                              if (!canManageLeadFlow) {
                                toast.error("Bạn không có quyền tạo hợp đồng từ Lead.");
                                return;
                              }
                              setMaintenanceContractLead(l);
                            }}
                          >
                          ...
                          <DropdownMenuItem
                            ...
                            onClick={() => {
                              if (!canSalesCare) {
                                toast.error("Bạn không có quyền đóng Lead.");
                                return;
                              }
                              handleStatusUpdate(l, "lost");
                            }}
                          >
