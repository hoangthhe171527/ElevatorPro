...
import { initiateWarrantyFlow, handleEquipmentArrival } from "@/lib/workflow-utils";
import {
  CreateJobModal,
...
export function MobileProjectDetail({
  ...
  return (
    <AppShell>
      ...
      <div
        className={cn(
          "mx--4 px-4 pt-4 pb-8 mb-4 relative overflow-hidden rounded-b-[2rem] shadow-xl",
          isCompleted ? "bg-emerald-600 shadow-emerald-500/20" : "bg-slate-900 shadow-slate-900/10",
        )}
      >
        ...
        <div className="relative z-10 pr-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span
              className={cn(
                "px-2 py-0.5 rounded uppercase font-black text-[9px] tracking-widest shadow-sm border",
                isCompleted
          ...
          <div className="flex flex-col gap-1 text-white/70 text-[11px] font-bold">
            <span className="flex items-center gap-1.5">
              <Calendar className="h-3 w-3" /> Bắt đầu: {formatDate(project.startDate)}
            </span>
            <span className="flex items-start gap-1.5 line-clamp-1">
      ...
      <div className="flex flex-col gap-4 pb-8 relative z-20 -mt-8 mx-4">
        ...
        {!readonly && (
          <div className="grid gap-2">
            {project.stage === "completion" && (
              <Button
                size="lg"
                className="h-14 font-black uppercase tracking-wider text-[11px] rounded-2xl bg-success text-white shadow-md shadow-success/20"
                onClick={() => setShowAccountantPayment(3)}
              >
            ...
            {project.stage === "waiting_for_equipment" && (
              <Button
                size="lg"
                className="h-14 font-black uppercase tracking-wider text-[11px] rounded-2xl bg-orange-600 hover:bg-orange-700 text-white shadow-md shadow-orange-500/20"
            ...
            <Button
              size="lg"
              variant="outline"
              className="h-12 font-black uppercase tracking-wider text-[10px] rounded-xl bg-white shadow-sm border-slate-200 text-slate-800"
        ...
        <div className="bg-primary/5 rounded-[24px] p-5 border border-primary/10 flex items-center justify-between">
          <div>
            <div className="text-[10px] font-black text-primary/70 uppercase tracking-widest mb-1">
              Phase hiện tại
            </div>
        ...
        <div className="bg-white rounded-[24px] p-5 shadow-[0_2px_10px_rgba(0,0,0,0.03)] border border-slate-100">
          ...
          <div className="space-y-3">
            {mockInvoices
              .filter((inv) => inv.targetId === project.id)
              .map((inv) => (
                <div
                  key={inv.id}
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div
                      className={`text-[13px] font-black ${inv.status === "paid" ? "text-emerald-500" : "text-slate-400"}`}
                    >
                      {formatVND(inv.amount)}
                    </div>
                  </div>
                  <div
                    className={`text-[9px] uppercase font-black tracking-widest ${inv.status === "paid" ? "text-emerald-500" : "text-slate-400"}`}
                  >
            ...
            {mockInvoices.filter((inv) => inv.targetId === project.id).length === 0 && (
              <div className="text-[11px] text-slate-400 font-bold italic text-center bg-slate-50 py-4 rounded-xl border border-slate-100 border-dashed">
                Chưa có xuất HĐ
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-[24px] shadow-[0_2px_10px_rgba(0,0,0,0.03)] border border-slate-100 px-5 py-2">
            {INSTALL_STAGES_TEMPLATE.map((stage, idx) => {
              ...
              return (
                <div
                  key={stage.id}
                  className="relative py-5 border-b border-slate-100 last:border-0 flex items-start gap-4"
                >
                  <div
                    className={cn(
                  ...
                  <div className="flex-1 min-w-0">
                    <h4
                      className={cn(
                        "font-black tracking-tight mb-1 line-clamp-1",
                        isCurrent ? "text-primary text-[15px]" : "text-[13px] text-slate-800",
                      )}
                    >
                    ...
                    {job ? (
                      <div className="flex flex-col gap-2 mt-2">
                        <StatusBadge
                          ...
                          className="w-fit scale-90 origin-left text-[9px] px-2 py-0.5"
                        >
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          >
                            <Button
                              size="sm"
                              variant="ghost"
                              className="h-7 px-3 bg-slate-50 text-[9px] font-black uppercase border border-slate-200 rounded-lg"
                            >
                      ...
                      <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest italic mt-2">
                        Chưa giao thợ
                      </div>
                    )}
                  </div>
      ...
      <ConfirmationDialog
        ...
        title="Hàng về công trình"
        description="Kích hoạt các công việc lắp đặt cơ khí."
        onConfirm={() => {
      ...
      {showAccountantPayment && (
        <AccountantPaymentModal
          ...
          contract={mockContracts.find(
            (c: Contract) => c.id === project.contractId || c.projectId === project.id,
          )}
          onSuccess={(amount) => {
      ...
      {showCEOApproval && (
        <CEOFinalApprovalModal
          ...
          enteredTotal={(() => {
            ...
            return (
              (contract.paymentStages.stage1Paid || 0) +
