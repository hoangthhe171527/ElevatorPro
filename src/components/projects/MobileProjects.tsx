    ...
    <AppShell>
      ...
      <div className="flex flex-col gap-4 pb-8">
        {projects.map((proj) => {
          ...
          return (
            <div key={proj.id} className="relative bg-white p-5 rounded-[24px] shadow-[0_2px_10px_rgba(0,0,0,0.03)] border border-slate-100/50">
               {/* Header Area */}
               <div className="flex justify-between items-start mb-4">
                  <div className="flex gap-3">
                     ...
                     <div>
                        <h3 className="font-black text-[15px] text-slate-800 leading-tight mb-1 pr-6">{proj.name}</h3>
                        <p className="text-[11px] font-medium text-slate-500 line-clamp-1">{proj.address}</p>
                     </div>
                  </div>
                  {isCompleted && (
                    <Badge variant="outline" className="bg-emerald-50 text-emerald-600 border-none px-2 py-0.5 rounded-full text-[9px] uppercase font-black uppercase absolute right-4 top-4">
                      Hoàn tất
                    </Badge>
                  )}
               </div>
               ...
               <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 mb-4">
                  <div className="flex justify-between items-end mb-2">
                     <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Tiến độ kỳ {currentStageIndex + 1}/8</span>
                     <span className="text-[12px] font-black text-primary">{PROJECT_STAGE_LABELS[proj.stage]}</span>
                  </div>
                        ...
                        return (
                          <div 
                             ...
                             className={cn(
                               ...
                               isPast ? "bg-primary" :
                               isCurrent ? "bg-primary/50" : "bg-slate-200"
                             )}
                          />
                        )
                     })}
               ...
               <div className="flex gap-2">
                  <Link to="/app/admin/projects/$projectId" params={{ projectId: proj.id }} className="flex-1">
                    <Button variant="outline" className="w-full h-10 rounded-xl text-[11px] font-black uppercase text-slate-600 bg-white border-slate-200 hover:bg-slate-50 shadow-sm">
                      Mở dự án
                    </Button>
                  </Link>
