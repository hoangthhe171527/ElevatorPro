...
export function WebJobsMonitor({
  tab: routeTab,
  initialPriority,
  initialStatus,
}: {
  ...
  const tabFiltered = useMemo(() => {
    if (tab === "pending")
      return baseFiltered.filter((j) => j.status === "pending" && j.type !== "install");
    // For specific types
    ...
    if (tab === "incident")
      return baseFiltered.filter(
        (j) => j.type === "incident" || j.title.toLowerCase().includes("sự cố"),
      );
    return [];
  }, [baseFiltered, tab]);
  ...
  const renderJobList = (jobs: typeof mockJobs, hideActions = false) => {
    ...
    const getTabColor = (type: string) => {
      switch (type) {
        ...
        case "warranty":
          return "border-l-green-600 bg-green-50/30";
        case "repair":
    ...
    return (
      <div className="bg-slate-50/20">
        {Object.entries(groups).map(([key, groupJobs]) => {
          ...
          const groupTitle = project
            ? `Dự án: ${project.name}`
            : `Khách hàng: ${customer?.name || "Khác"}`;
                  ...
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <Badge
                          variant="secondary"
                          className="text-[9px] font-black px-1.5 h-4 bg-slate-100 text-slate-600"
                        >
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        description={
          isIsolated
            ? `Giám sát tiến độ thực hiện mảng ${(tabLabels[tab] || "Công việc").replace("Theo dõi ", "")}`
            : "CEO giám sát tiến độ thực hiện của các đội nhóm hiện trường"
        }
        actions={
          ...
          {!isIsolated && (
            <div className="border-b bg-muted/20 px-4 pt-4">
              <TabsList className="grid w-full grid-cols-5 h-12 bg-transparent gap-2 p-0">
                <TabsTrigger
                  value="maintenance"
                  className="data-[state=active]:bg-background data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-blue-500 rounded-none h-full border-b-2 border-transparent font-bold text-[10px] uppercase"
                >
                  Bảo trì
                </TabsTrigger>
                <TabsTrigger
                  value="warranty"
                  className="data-[state=active]:bg-background data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-green-600 rounded-none h-full border-b-2 border-transparent font-bold text-[10px] uppercase"
                >
                  Bảo hành
                </TabsTrigger>
                <TabsTrigger
                  value="repair"
                  className="data-[state=active]:bg-background data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-red-500 rounded-none h-full border-b-2 border-transparent font-bold text-[10px] uppercase"
                >
                  Sửa chữa
                </TabsTrigger>
                <TabsTrigger
                  value="inspection"
                  className="data-[state=active]:bg-background data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-indigo-500 rounded-none h-full border-b-2 border-transparent font-bold text-[10px] uppercase"
                >
                  Khảo sát
                </TabsTrigger>
                <TabsTrigger
                  value="incident"
                  className="data-[state=active]:bg-background data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-rose-500 rounded-none h-full border-b-2 border-transparent font-bold text-[10px] uppercase relative"
                >
                  Sự cố QR
              ...
              {mockIssues.map((issue) => {
                ...
                return (
                  <div key={issue.id} className="p-4 hover:bg-muted/10 transition-colors group">
                    <div className="flex items-center gap-4">
                      <div
                        className={`h-10 w-10 shrink-0 rounded-lg flex items-center justify-center ${issue.status === "open" ? "bg-destructive/10 text-destructive" : "bg-primary/10 text-primary"}`}
                      >
                        {issue.status === "open" ? (
