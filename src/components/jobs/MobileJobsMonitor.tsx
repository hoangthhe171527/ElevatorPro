...
export function MobileJobsMonitor({ tab: routeTab }: { tab: string }) {
    ...
    return (
      <div className="flex flex-col gap-3 pb-8">
        {jobs.map((j) => {
          ...
          return (
            <div key={j.id} className="block relative bg-white p-4 rounded-3xl shadow-[0_2px_10px_rgba(0,0,0,0.03)] border border-slate-100">
               <div className="absolute right-4 top-4">
                  <Button variant="ghost" size="icon" className="h-8 w-8 text-primary bg-primary/5 rounded-full" onClick={(e) => {
                    e.preventDefault();
                    setSelectedJob(j);
               ...
               <Link to="/app/admin/jobs/$jobId" params={{ jobId: j.id }} search={{ tab: tab, readonly: "true" }}>
                  <div className="flex items-start gap-4 pr-10">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1.5">
                         <span className={cn("text-[9px] font-black tracking-widest px-1.5 py-0.5 rounded-full uppercase", isUrgent ? "bg-rose-600 text-white" : "bg-slate-100 text-slate-500")}>
                           {j.code.split("-")[1]}
                         </span>
                         <h4 className="font-bold text-[14px] truncate leading-tight text-slate-800">{j.title}</h4>
                      </div>

                      <div className="flex items-center gap-2 mb-3">
                        <span className={cn(
                          "text-[9px] font-black uppercase px-2 py-0.5 rounded-full border", 
                          isUrgent ? "text-rose-600 border-rose-200 bg-rose-50" : "text-slate-500 border-slate-200"
  ...
  const DropdownIcon = ({ icon: Icon }: any) => (
    <div className="w-4 flex justify-center"><Icon className="h-3 w-3 opacity-70" /></div>
  )
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        actions={
          <Button size="sm" className="h-8 rounded-full px-3 text-[11px] uppercase font-black" onClick={() => setQuickIncidentOpen(true)}>
                  ...
                  <div className="flex items-center gap-4">
                     ...
                     <div className="flex-1 min-w-0">
                       <div className="flex justify-between items-start mb-1">
                          ...
                          <Badge variant="secondary" className="text-[9px] tracking-tight shrink-0 px-1.5 bg-orange-100 text-orange-700">KỲ {p.stage}</Badge>
                       </div>
                       ...
                       <div className="mt-3">
                          <div className="flex justify-between items-end mb-1.5">
                             <span className="text-[9px] font-black uppercase text-orange-600 tracking-widest">Tiến độ thi công</span>
                          </div>
                          <div className="flex gap-1">
                            {Array.from({ length: 8 }).map((_, i) => (
                              <div key={i} className={`h-1.5 flex-1 rounded-full ${i === 1 ? 'bg-orange-500' : i < 1 ? 'bg-orange-500/30' : 'bg-slate-100'}`} />
                            ))}
                          </div>
                       </div>
                     </div>
