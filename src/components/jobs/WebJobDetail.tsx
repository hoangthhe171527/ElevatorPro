...
export function WebJobDetail({ job, readonly = false }: { job: Job; readonly?: boolean }) {
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-sm font-medium opacity-90 mt-0.5">
                Công việc này chưa có kỹ thuật viên phụ trách. Vui lòng chọn thợ phù hợp để bắt đầu.
              </p>
            </div>
          </div>
          <Button
        ...
        <div className="mb-6 p-4 bg-blue-50/50 border border-blue-100 rounded-xl flex items-center gap-3 text-blue-700 shadow-sm">
          ...
          <div>
            ...
            <div className="text-xs opacity-80 mt-0.5">
              Mọi thao tác thay đổi dữ liệu đã bị khóa để đảm bảo tính minh bạch.
            </div>
          </div>
        </div>
      )}
            ...
            return (
              <div key={s.id} className="flex items-center flex-1">
                <div className="flex flex-col items-center gap-2 min-w-[120px] relative z-10 w-full">
                  <div
                    className={`h-12 w-12 rounded-2xl flex items-center justify-center border-[3px] transition-all duration-500 shadow-sm ${
                      isDone
                        ? "bg-primary border-primary text-white scale-110"
                        : isCurrent
                          ? "bg-white border-primary text-primary animate-pulse shadow-primary/20 scale-110"
                  ...
                  <span
                    className={`text-[11px] uppercase tracking-widest font-bold mt-1 ${isCurrent ? "text-primary" : isDone ? "text-slate-800" : "text-slate-400"}`}
                  >
                    {s.label}
                  </span>
                </div>
                {idx < arr.length - 1 && (
                  <div
                    className={`h-1.5 flex-1 rounded-full mx-2 -ml-6 -mr-6 z-0 ${isDone ? "bg-primary" : "bg-slate-100"}`}
                  />
                )}
              </div>
            );
          ...
          <Card className="p-8 shadow-sm border-slate-100 rounded-3xl">
                ...
                <div>
                  ...
                  <div className="font-bold text-[15px] text-slate-800">
                    {job.completedAt ? (
                      ...
                      <span className="text-slate-400 italic font-medium mt-1 inline-block">
                        Chưa đo lường
                      </span>
                    )}
                  </div>
            ...
            <div className="mt-8 pt-8 border-t border-slate-100">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {job.tenantId === "t-1" ? (
                  <div
                    className={`p-6 rounded-2xl border flex items-center justify-between ${isApproved ? "bg-emerald-50/50 border-emerald-200" : "bg-slate-50 border-slate-200"}`}
                  >
                    <div className="flex items-center gap-4">
                      <div
                        className={`h-12 w-12 rounded-xl flex items-center justify-center ${isApproved ? "bg-emerald-500 text-white shadow-md shadow-emerald-500/20" : "bg-white border border-slate-200 text-slate-400"}`}
                      >
                      ...
                      <div>
                        ...
                        <div className="text-xs text-slate-500 font-medium mt-0.5">
                          {isApproved
                            ? "Đã được kiểm tra bởi Trưởng phòng"
                            : "Trạng thái: Đang chờ"}
                        </div>
                      </div>
                    </div>
                    {!readonly && !isApproved && job.report && (
                      <Button
                        ...
                        className="bg-primary hover:bg-primary/90 text-white font-bold h-10 px-6 rounded-xl shadow-md"
                      >
                        Duyệt Báo cáo
                      </Button>
                    )}
                  ...
                  <div
                    className={`col-span-2 p-8 rounded-3xl border-2 border-dashed flex flex-col items-center justify-center gap-5 ${status === "completed" ? "bg-emerald-50/50 border-emerald-200" : "bg-blue-50 border-blue-200"}`}
                  >
                    {status === "completed" ? (
                      <>
                        ...
                        <div className="text-center">
                          ...
                          <div className="text-sm text-slate-600 font-medium mt-1">
                            Dữ liệu đã được lưu trữ vĩnh viễn trên đám mây.
                          </div>
                        </div>
                      </>
                    ) : (
                      <>
                        ...
                        <div className="text-center">
                          <div className="font-black text-blue-800 text-lg">
                            Không yêu cầu duyệt tay (Auto-Pass)
                          </div>
                          <div className="text-sm text-slate-600 font-medium mt-1">
                            Khách thợ chỉ báo cáo là hệ thống sẽ đóng job cho công ty nhỏ.
                          </div>
                        </div>
                        {!readonly && job.report && (
                          <Button
                            ...
                            className="h-12 px-8 rounded-xl font-black bg-blue-600 hover:bg-blue-700 text-white shadow-md"
                          >
                            Bấm khép hồ sơ ngay
                          </Button>
                        )}
                ...
                <div className="mt-6 p-5 rounded-2xl bg-indigo-50 border border-indigo-100 flex items-start gap-4 text-indigo-800 shadow-inner">
                  ...
                  <div className="text-[13px] leading-relaxed font-medium">
                    <span className="font-bold uppercase tracking-wider block mb-1">
                      Quy định thay thế linh kiện
                    </span>
                    Nếu phát sinh hỏng hóc trong báo cáo bảo trì, bạn KHÔNG ĐƯỢC làm chung hóa đơn.
                    Phải bấm "Tạo task sửa chữa" để hạch toán độc lập nhằm đảm bảo đúng luật kế
                    toán.
                  </div>
                </div>
              )}
            ...
            <Card className="p-6 shadow-sm border-slate-100 rounded-3xl bg-slate-800 text-white">
              ...
              <div className="p-5 rounded-2xl bg-white/5 border border-white/10 hover:bg-white/10 transition-colors">
                <div className="flex items-center gap-4 mb-4 pb-4 border-b border-white/10">
                  ...
                  <div>
                    <div className="font-black text-[11px] text-slate-400 uppercase tracking-widest mb-1">
                      Mã định danh định vị
                    </div>
                    <div className="font-mono font-black text-xl tracking-tight text-white">
                      {elevator.code}
                    </div>
                  </div>
                </div>
                ...
                >
                  <Button
                    variant="secondary"
                    className="w-full h-10 rounded-xl bg-white text-slate-900 font-black uppercase text-[10px] hover:bg-slate-100"
                  >
