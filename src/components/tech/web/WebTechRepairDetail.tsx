        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 mb-8">
           ...
           <div className="flex items-center gap-3">
              ...
              {status === "scheduled" && !readonly && (
                 <Button 
                    onClick={() => { setStatus("in_progress"); toast.success("Bắt đầu xử lý!"); }}
              ...
              <Card className="rounded-[2.5rem] border-none shadow-xl shadow-slate-200/50 overflow-hidden">
                ...
                <CardContent className="p-8 space-y-8">
                   <div className="space-y-4">
                      <label className="text-sm font-black uppercase tracking-widest text-slate-400">Mô tả tình trạng hư hỏng</label>
                      <Textarea 
                   ...
                   <div className="grid grid-cols-2 gap-8">
                      <div className="space-y-4">
                         <div className="flex items-center justify-between">
                            <label className="text-sm font-black uppercase tracking-widest text-slate-400">Ảnh hiện trạng ({beforePhotos.length})</label>
                            {status === "in_progress" && (
                               <Button variant="ghost" size="sm" onClick={() => setBeforePhotos([...beforePhotos, "/placeholder.svg"])} className="text-primary font-bold">
                         ...
                         <div className="grid grid-cols-2 gap-4">
                            {beforePhotos.map((p, i) => (
                               <div key={i} className="aspect-video bg-slate-100 rounded-2xl border-2 border-dashed border-slate-200 flex items-center justify-center relative group overflow-hidden">
                                  <Camera className="h-8 w-8 text-slate-300" />
                                  <div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                     <Button variant="destructive" size="icon" className="h-8 w-8 rounded-full" onClick={() => setBeforePhotos(beforePhotos.filter((_, idx) => idx !== i))}>
                      ...
                      <div className="space-y-4">
                         <div className="flex items-center justify-between">
                            <label className="text-sm font-black uppercase tracking-widest text-slate-400">Ảnh sau khi sửa ({afterPhotos.length})</label>
                            {status === "in_progress" && (
                               <Button variant="ghost" size="sm" onClick={() => setAfterPhotos([...afterPhotos, "/placeholder.svg"])} className="text-primary font-bold">
                                  <Plus className="h-4 w-4 mr-1" /> Thêm ảnh
                               </Button>
                            )}
                         </div>
                   ...
                   <div className="space-y-6">
                      <div className="flex gap-2 mb-6">
                         <Select defaultValue="" onValueChange={addPart}>
                            <option value="" disabled>-- Chọn linh kiện cần thay --</option>
                            {mockInventory.slice(0, 10).map(item => (
                               <option key={item.id} value={item.id}>{item.name} - {formatVND(item.unitPrice)}</option>
                            ))}
                         </Select>
                      </div>
                      ...
                      <div className="rounded-[2rem] border overflow-hidden">
                         <table className="w-full text-sm">
                            <thead className="bg-slate-50 border-b">
                               <tr>
                                  <th className="px-6 py-4 text-left font-black uppercase text-[10px] tracking-widest text-slate-400">Linh kiện</th>
                                  <th className="px-6 py-4 text-center font-black uppercase text-[10px] tracking-widest text-slate-400">Số lượng</th>
                                  <th className="px-6 py-4 text-right font-black uppercase text-[10px] tracking-widest text-slate-400">Đơn giá</th>
                                  <th className="px-6 py-4 text-right font-black uppercase text-[10px] tracking-widest text-slate-400">Thành tiền</th>
                            ...
                            <tbody className="divide-y">
                                  ...
                                  <tr key={p.id}>
                                     ...
                                     <td className="px-6 py-4 text-center font-bold">
                                        <div className="flex items-center justify-center gap-2">
                                           <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => setParts(parts.map(x => x.id === p.id ? { ...x, quantity: Math.max(1, x.quantity - 1) } : x))}>-</Button>
                                           <span>{p.quantity}</span>
                                           <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => setParts(parts.map(x => x.id === p.id ? { ...x, quantity: x.quantity + 1 } : x))}>+</Button>
                                        </div>
                                     </td>
                                     <td className="px-6 py-4 text-right font-bold text-slate-500">{formatVND(p.price)}</td>
                                     <td className="px-6 py-4 text-right font-black text-slate-900">{formatVND(p.price * p.quantity)}</td>
                                     <td className="px-6 py-4">
                                        <Button variant="ghost" size="icon" className="h-8 w-8 text-red-400 hover:text-red-600" onClick={() => removePart(p.id)}>
                                           <Trash2 className="h-4 w-4" />
                                        </Button>
                                     </td>
                                  </tr>
                               ...
                               {parts.length === 0 && (
                                  <tr>
                                     <td colSpan={5} className="px-6 py-12 text-center text-slate-400 font-bold italic">Chưa chọn vật tư nào</td>
                                  </tr>
                               )}
                            </tbody>
                            <tfoot className="bg-slate-50/50">
                               <tr>
                                  <td colSpan={3} className="px-6 py-6 text-right font-black uppercase tracking-widest text-slate-400">Tổng cộng ước tính</td>
                                  <td className="px-6 py-6 text-right font-black text-2xl text-primary">{formatVND(totalAmount)}</td>
                                  <td></td>
                               </tr>
                            </tfoot>
                         </table>
              ...
              <Card className="rounded-[2.5rem] border-none shadow-xl shadow-slate-200/40 p-8 bg-slate-900 text-white overflow-hidden relative">
                ...
                <div className="space-y-6 relative z-10">
                  ...
                  <div>
                    ...
                    <div className="text-sm font-black text-primary">{elevator?.brand} - {elevator?.model}</div>
                    <div className="text-[10px] text-slate-400 mt-1 uppercase font-bold">Mã thang: {elevator?.code}</div>
                  </div>
                  <div className="pt-4 border-t border-white/10">
                     <div className="text-[10px] text-slate-400 uppercase font-bold mb-3">Tình trạng bảo hành</div>
                 ...
                 <div className="space-y-6">
                    <div className="flex gap-4">
                       ...
                       <div>
                          ...
                          <div className="text-[10px] text-slate-400 font-bold uppercase">12/01/2024 • Thợ: Lâm CEO</div>
                       </div>
                    </div>
                    <div className="flex gap-4 opacity-50">
                       ...
                       <div className="text-sm font-bold italic text-slate-400">Không có dữ liệu khác</div>
                    </div>
                 </div>
              </Card>
           </div>
...
function Select({ children, defaultValue, onValueChange, className }: any) {
  return (
    <select 
      className={cn("h-12 w-full rounded-2xl border-slate-200 bg-white px-4 font-bold text-slate-700 shadow-sm focus:ring-2 focus:ring-primary/20 transition-all", className)}
