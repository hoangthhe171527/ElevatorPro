...
export function WebMaintenanceJobDetail({ job, readonly = false }: { job: Job; readonly?: boolean }) {
           ...
           <div className="flex items-center gap-3">
              ...
              {status === "completed" && (
                <div className="flex items-center gap-3 bg-emerald-50 text-emerald-600 px-6 py-3 rounded-2xl border border-emerald-100 shadow-sm">
                   ...
                   <span className="font-black uppercase tracking-widest text-xs">Đã hoàn thành bàn giao</span>
                </div>
              )}
           </div>
        </div>
                    ...
                    {steps.map((step) => (
                      ...
                      >
                         ...
                         <div className="flex-1">
                            <div className={cn("font-black text-[15px] mb-1 transition-colors", step.resolved ? "text-emerald-900" : "text-slate-800 group-hover:text-primary")}>
                               {step.label}
                            </div>
                         ...
                         {step.resolved && (
                            <div className="flex items-center gap-2">
                               <Badge className="bg-emerald-100 text-emerald-700 border-none font-black text-[9px] uppercase">OK</Badge>
              ...
              <Card className="rounded-[2.5rem] border-none shadow-xl shadow-slate-200/40 bg-white p-8">
                 ...
                 <div className="mt-6 p-5 rounded-2xl bg-amber-50 border border-amber-100 flex items-start gap-4 text-amber-800">
                    ...
                    <p className="text-xs font-bold leading-relaxed">
                       Nếu phát sinh hư hỏng cần thay thế linh kiện, vui lòng tạo riêng một "Yêu cầu sửa chữa" để đảm bảo đúng quy trình hạch toán vật tư.
                    </p>
                 </div>
              </Card>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                 <Card className="rounded-[2.5rem] border-none shadow-xl shadow-slate-200/40 bg-white p-8">
                    ...
                    <div className="aspect-video rounded-3xl bg-slate-50 border-2 border-dashed border-slate-200 flex flex-col items-center justify-center text-slate-400 group hover:border-primary transition-colors cursor-pointer">
                       ...
                       <span className="text-[10px] font-black uppercase tracking-widest">Tải ảnh hiện trường</span>
                    </div>
                 </Card>
                 <Card className="rounded-[2.5rem] border-none shadow-xl shadow-slate-200/40 bg-white p-8">
                    <h3 className="font-black text-[11px] uppercase tracking-widest text-slate-400 mb-6 flex items-center gap-2">
              ...
              <Card className="rounded-[2.5rem] border-none shadow-xl shadow-slate-200/40 bg-slate-900 text-white p-8 overflow-hidden relative">
                 ...
                 <div className="flex items-center gap-5 relative z-10">
                    ...
                    <div>
                       ...
                       <div className="text-xs font-bold text-slate-500 uppercase tracking-widest">Team Kỹ thuật Bảo trì</div>
                    </div>
                 </div>
                 <div className="mt-8 pt-8 border-t border-white/5 space-y-4 relative z-10">
                    ...
                    <div className="flex items-center justify-between">
                       ...
                       <StatusBadge variant={status === 'completed' ? 'success' : 'warning'} className="h-5 text-[9px]">
                 ...
                 {elevator && (
                   <div className="space-y-6">
                      <div className="p-5 rounded-3xl bg-slate-50 border border-slate-100">
                         <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Mã thang máy</div>
                         <div className="text-2xl font-black text-slate-800 font-mono tracking-tight">{elevator.code}</div>
                      </div>
                      <div className="space-y-4 px-2">
                         ...
                         <div className="flex justify-between items-center text-sm">
                            ...
                            <span className="font-black text-slate-800 text-right">{elevator.building}</span>
                         </div>
                      </div>
                      <div className="pt-6 border-t border-slate-50">
                         <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Lịch sử bảo trì</div>
                         <div className="space-y-3">
                            <div className="flex items-center gap-3">
                               <div className="h-2 w-2 rounded-full bg-emerald-500" />
                               <span className="text-xs font-bold text-slate-600">Lần cuối: {formatDateTime(elevator.lastMaintenance).split(',')[0]}</span>
                            </div>
                            <div className="flex items-center gap-3">
                               <div className="h-2 w-2 rounded-full bg-primary animate-pulse" />
                               <span className="text-xs font-black text-primary">Tiếp theo: {formatDateTime(elevator.nextMaintenance).split(',')[0]}</span>
                            </div>
                         </div>
                      </div>
                      <Link to="/admin/elevators/$elevatorId" params={{ elevatorId: elevator.id }} className="block pt-4">
                        <Button variant="ghost" className="w-full text-primary font-black uppercase text-[10px] gap-2">
              ...
              <Card className="rounded-[2.5rem] border-none shadow-xl shadow-slate-200/40 bg-white p-8">
                 ...
                 {customer && (
                   <div className="space-y-4">
                      ...
                      <Button variant="outline" className="w-full h-10 rounded-xl border-slate-200 text-slate-600 font-black uppercase text-[10px]">
                         Liên hệ Khách hàng
                      </Button>
                   </div>
                 )}
