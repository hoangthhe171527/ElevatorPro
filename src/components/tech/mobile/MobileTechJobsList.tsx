...
export function MobileTechJobsList({ search }: { search: { tab?: string } }) {
  ...
  const myProjectIds = useMemo(() => Array.from(new Set(myJobs.map(j => j.projectId).filter(Boolean))), [myJobs]);
  const myProjects = useMemo(() => mockProjects.filter(p => myProjectIds.includes(p.id)), [myProjectIds]);

  const baseFiltered = useMemo(() => {
    return myJobs.filter((j) => {
      const m1 = !searchQuery || j.title.toLowerCase().includes(searchQuery.toLowerCase()) || j.code.toLowerCase().includes(searchQuery.toLowerCase());
    ...
    <AppShell secondaryNav={
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="flex gap-2">
            ...
            <Button variant="outline" size="icon" className="h-11 w-11 rounded-2xl bg-white border-slate-100 shadow-sm">
        ...
        {tab === "install" ? (
          <div className="space-y-4">
             {myProjects.map(project => {
                const projectJobs = myJobs.filter(j => j.projectId === project.id && j.type === 'install');
                  ...
                  >
                    <Card className="p-5 rounded-[28px] border-none shadow-xl shadow-slate-900/5 hover:scale-[1.02] transition-all overflow-hidden relative group">
                       ...
                       <div className="flex justify-between items-start mb-4">
                          <Badge variant="outline" className="text-[10px] uppercase font-black tracking-widest border-primary/20 text-primary bg-primary/5 px-2 py-0.5">Dự án</Badge>
                          <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">#{project.id}</div>
                       </div>
                       
                       <h3 className="text-lg font-black text-slate-800 leading-tight mb-2">{project.name}</h3>
                       
                       <div className="space-y-4 mt-6">
                          <div className="flex justify-between items-end">
                             <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Tiến độ thi công</span>
                          ...
                          <div className="flex items-center justify-between pt-2">
                             <div className="flex -space-x-1">
                                {Array.from({ length: totalCount }).map((_, i) => (
                                   <div key={i} className={cn("h-1.5 w-4 rounded-full mr-1", i < completedCount ? "bg-success" : "bg-slate-200")} />
                                ))}
                             </div>
                             <div className="flex items-center gap-1 text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                                {completedCount}/{totalCount} Giai đoạn <ChevronRight className="h-3 w-3" />
                             </div>
                          </div>
                       </div>
                    </Card>
          ...
          <div className="space-y-3">
                    ...
                    <Card className="p-4 rounded-[24px] border-none shadow-lg shadow-slate-900/5 items-center flex gap-4 hover:bg-slate-50 transition-colors">
                       ...
                       <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-0.5">
                             <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">ID: {job.code.split('-').pop()}</span>
                             <StatusBadge variant={jobStatusVariant[job.status]} className="text-[8px] px-1.5 py-0 h-4">{jobStatusLabel[job.status]}</StatusBadge>
                          </div>
             ...
             {tabFiltered.length === 0 && (
               <div className="p-12 text-center">
                  ...
                  <p className="text-xs text-slate-400">Bạn hiện không có nhiệm vụ nào trong mục này.</p>
               </div>
             )}
          </div>
        )}
