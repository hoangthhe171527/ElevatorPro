...
import { mockJobs, getCustomer, formatDate, type Project } from "@/lib/mock-data";
import {
  ArrowLeft,
  Calendar,
  ChevronRight,
    ...
    >
      <div className="bg-slate-900 px-4 pt-4 pb-12 rounded-b-[2.5rem] shadow-2xl shadow-slate-900/20 relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-4">
          <div className="flex items-center gap-2">
            <Badge className="bg-orange-500 text-white border-none text-[9px] font-black tracking-widest px-2 py-0.5">
              INSTALLATION
            </Badge>
      ...
      <div className="px-4 -mt-6 space-y-4 pb-12">
          ...
          <div className="flex justify-between items-end">
            <div className="space-y-1">
              ...
              <div className="text-2xl font-black text-slate-800">
                Giai đoạn {projectJobs.filter((j) => j.status === "completed").length}/8
              </div>
            </div>
            <div className="h-14 w-14 rounded-full border-4 border-slate-50 flex items-center justify-center relative">
              <svg className="h-12 w-12 -rotate-90">
                ...
                <circle
                  ...
                  strokeDasharray={`${(projectJobs.filter((j) => j.status === "completed").length / 8) * 125.6} 125.6`}
                  strokeLinecap="round"
                />
              </svg>
              <span className="absolute text-[10px] font-black">
                {Math.round((projectJobs.filter((j) => j.status === "completed").length / 8) * 100)}
                %
              </span>
            </div>
          </div>
        ...
        <div className="space-y-4">
          <h3 className="text-xs font-black uppercase tracking-[0.1em] text-slate-800 ml-2">
            Trình tự 8 giai đoạn
          </h3>
              ...
              return (
                <Card
                  ...
                  className={cn(
                    ...
                    isCurrent
                      ? "shadow-xl shadow-primary/10 ring-2 ring-primary bg-white"
                      : isCompleted
                        ? "bg-emerald-50/50 shadow-sm opacity-80"
                  ...
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <h4
                        className={cn(
                          ...
                          isCurrent ? "text-primary" : "text-slate-800 font-bold",
                        )}
                      >
                        {stageName.split(": ")[1]}
                      </h4>
                    ...
                    {isCurrent && job ? (
                      <Link to="/app/tech/jobs/$jobId" params={{ jobId: job.id }}>
                        <Button
                          size="sm"
                          className="h-8 rounded-xl text-[10px] font-black uppercase shadow-md shadow-primary/20"
                        >
                          THỰC HIỆN
                        </Button>
                      </Link>
