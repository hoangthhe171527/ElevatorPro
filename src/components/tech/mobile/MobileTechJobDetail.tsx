  ...
  return (
    <AppShell secondaryNav={
      <div className="flex items-center justify-between px-4 py-2 bg-slate-900 text-white">
        <Link to="/app/tech/jobs" search={{ tab: job.type } as any} className="h-8 w-8 rounded-full bg-white/10 flex items-center justify-center">
            <ArrowLeft className="h-4 w-4" />
        </Link>
        <div className="flex flex-col items-center">
           <span className="text-[10px] font-black uppercase tracking-[0.2em] opacity-50">Chi tiết công việc</span>
      ...
      <div className="bg-slate-900 px-4 pt-4 pb-12 rounded-b-[2.5rem] shadow-2xl shadow-slate-900/20 relative overflow-hidden">
         ...
         <div className="relative z-10 space-y-4">
            <div className="flex items-center gap-2">
               <StatusBadge variant={priorityVariant[job.priority]} className="text-[9px] uppercase font-black px-2 py-0.5 h-auto leading-normal tracking-widest">{priorityLabel[job.priority]}</StatusBadge>
               <StatusBadge variant={jobStatusVariant[status]} className="text-[9px] uppercase font-black px-2 py-0.5 h-auto leading-normal tracking-widest bg-white/10 text-white border-none">{jobStatusLabel[status]}</StatusBadge>
            </div>
            ...
            <div className="flex items-center gap-4 text-white/50 text-[11px] font-bold">
               <div className="flex items-center gap-1"><Calendar className="h-3 w-3" /> {formatDateTime(job.scheduledFor)}</div>
            </div>
         </div>
      </div>
      ...
      <div className="px-4 -mt-6 space-y-4 pb-12">
            ...
            <Card className="p-5 rounded-[28px] border-none shadow-xl shadow-primary/10 bg-primary text-white">
               <div className="flex items-center justify-between mb-4">
                  <div className="flex flex-col">
                     <span className="text-[10px] font-black uppercase tracking-widest opacity-70">Trạng thái hiện tại</span>
               ...
               <div className="grid grid-cols-2 gap-3">
                  {status === "scheduled" ? (
                     <Button 
                       ...
                       onClick={() => { setStatus("in_progress"); toast.success("Bắt đầu thực hiện!"); }}
                     >
                        BẮT ĐẦU CÔNG VIỆC
                     </Button>
                  ) : (
         ...
         <div className="grid grid-cols-2 gap-3">
            <Card className="p-4 rounded-[24px] border-none shadow-lg shadow-slate-900/5 bg-white">
               ...
               <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 line-clamp-1">{cus?.name}</div>
               <a href={`tel:${cus?.phone}`} className="text-sm font-black text-indigo-600 block truncate">{cus?.phone}</a>
            </Card>
            <Card className="p-4 rounded-[24px] border-none shadow-lg shadow-slate-900/5 bg-white">
               ...
               <div className="text-[11px] font-black text-slate-800 line-clamp-2 leading-tight">Xem bản đồ <ChevronRight className="h-2 w-2 inline ml-0.5" /></div>
            </Card>
         </div>
         ...
         {job.type === "install" && (
            <Card className="p-5 rounded-[28px] border-none shadow-lg shadow-slate-900/5 bg-white">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Badge variant="outline" className="text-[9px] font-black opacity-50 px-1.5 py-0 h-4 border-slate-200">#{job.code.split('-').pop()}</Badge>
                </div>
                      ...
                      return (
                         <div key={idx} className="relative z-10 flex flex-col items-center">
                            <div className={cn(
                               ...
                               isPast ? "bg-success" : isCurrent ? "bg-primary scale-125 shadow-primary/30" : "bg-slate-200"
                            )} />
                            {isCurrent && (
                               <div className="absolute top-8 text-[8px] font-black uppercase text-primary whitespace-nowrap bg-primary/5 px-1 rounded shadow-sm">BẠN ĐANG Ở ĐÂY</div>
                            )}
                         </div>
                      );
                   })}
         ...
         <Card className="p-5 rounded-[28px] border-none shadow-lg shadow-slate-900/5 bg-white space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
               <div>
                  <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3">Trước khi sửa ({beforeCount})</div>
                  <Button variant="outline" className="w-full aspect-square rounded-[20px] flex-col gap-2 border-2 border-dashed border-slate-100 bg-slate-50 hover:bg-slate-100/50 text-slate-400" onClick={() => { setBeforeCount(c => c + 1); toast.success("Đã lưu ảnh Trước"); }}>
               ...
               <div>
                  <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3">Sau khi xong ({afterCount})</div>
                  <Button variant="outline" className="w-full aspect-square rounded-[20px] flex-col gap-2 border-2 border-dashed border-slate-100 bg-slate-50 hover:bg-slate-100/50 text-slate-400" onClick={() => { setAfterCount(c => c + 1); toast.success("Đã lưu ảnh Sau"); }}>
                     <div className="h-10 w-10 bg-white rounded-xl shadow-sm flex items-center justify-center">
                        <Camera className="h-5 w-5" />
                     </div>
                     <span className="text-[10px] font-black">CHỤP ẢNH</span>
         ...
         <Card className="p-5 rounded-[28px] border-none shadow-lg shadow-slate-900/5 bg-white">
            <div className="flex items-center gap-2 mb-4">
               ...
               <h3 className="text-xs font-black uppercase tracking-[0.1em] text-slate-800">Biên bản bàn giao</h3>
            </div>
            <Textarea
            ...
            {!isCompleted && (
               <div className="mt-4 p-3 rounded-xl bg-orange-50 border border-orange-100 flex items-start gap-2">
                  ...
                  <p className="text-[10px] font-black text-orange-700 uppercase tracking-wide leading-tight">Yêu cầu đổi lịch? Hãy liên hệ Leader ngay.</p>
               </div>
            )}
         </Card>
      </div>
