    ...
    <AppShell secondaryNav={
        <div className="flex items-center justify-between px-4 py-2 bg-slate-900 text-white">
          ...
          <div className="flex flex-col items-center">
             <span className="text-[10px] font-black uppercase tracking-[0.2em] opacity-50">Lịch làm việc</span>
                    ...
                    <Card className="p-4 rounded-[24px] border-none shadow-lg shadow-slate-900/5 bg-white relative overflow-hidden group hover:scale-[1.01] transition-all">
                       <div className="flex items-start gap-4">
                          <div className={cn(
                             ...
                             j.priority === 'urgent' ? "bg-rose-50 text-rose-500" : "bg-slate-50 text-slate-400"
                          )}>
                             <Briefcase className="h-5 w-5" />
                          </div>
                          <div className="flex-1 min-w-0">
                             <div className="flex items-center gap-2 mb-0.5">
                                <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">Ghé thăm: {formatDateTime(j.scheduledFor).split(",")[1]}</span>
                                <StatusBadge variant={priorityVariant[j.priority]} className="text-[8px] h-4 px-1.5">{priorityLabel[j.priority]}</StatusBadge>
                             </div>
                             <h4 className="font-black text-sm text-slate-800 truncate mb-1">{j.title}</h4>
